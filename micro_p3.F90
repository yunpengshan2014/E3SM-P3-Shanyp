...
!<shanyp 07112022
       if ((mincdnc.gt.0._rtype).and.(qc(k).ge.qsmall)) then
        nc_incld(k) = max(nc_incld(k),mincdnc*inv_rho(k))
        nc(k) = max(nc(k),mincdnc/inv_cld_frac_l(k)*inv_rho(k))
       end if
!shanyp 07112022>
...
!<shanyp 07112022
!      write(iulog,*) "LLILY, mincdnc=",mincdnc
      if ((mincdnc.gt.0._rtype).and.(qc(k).ge.qsmall)) then
       nc(k) = max(nc(k),mincdnc*cld_frac_l(k)/rho(k))
       nc_incld(k) = max(nc_incld(k),mincdnc/rho(k))
      end if
!shanyp 07112022>
